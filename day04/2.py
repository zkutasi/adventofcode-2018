...
bestofthebest = None
for t in range(60):
  (ids, slices) = zip ( * [ (g.guardid, g.sleepmatrix[t]) for g in guards ] )
  maxi = max(slices)
  idx = slices.index(maxi)
  bestguard_id = ids[idx]
  print "for t=%2d guard %5s slept %d times" % (t, bestguard_id, maxi)
  if bestofthebest is None:
    bestofthebest = (bestguard_id, maxi, t)
  elif maxi > bestofthebest[1]:
    bestofthebest = (bestguard_id, maxi, t)

print "Best answer is %d" % (int(bestofthebest[0]) * bestofthebest[2])
